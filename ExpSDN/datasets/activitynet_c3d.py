...
def create_loaders(loader_configs):
    ...
    for di,dt in enumerate(loader_configs.keys()):
        ...
        dsets[dt] = ACNET_C3D(loader_configs[dt])
...
class ACNET_C3D(Dataset):
    ...
    def __getitem__(self, index):
        ...
        ann = self.anns[index]
        if 'v_' in ann['video']:
            i3dfeat = self.i3dfeat[ann['video']]['c3d_features'][:] 
        else:
            i3dfeat = self.i3dfeat['v_'+ann['video']]['c3d_features'][:] 
        ...
        return index, i3dfeat, None, None, tokens, torch.from_numpy(loc_start), torch.from_numpy(loc_end), \
               torch.from_numpy(localization), time_start, time_end, factor, ann['duration'], raw_tokens
